...
function processTransaction(stockData: StrictStockDataType, amount: number, type: string): boolean {
	...
	// get player's total balance in own inventory
	const totalBalance = checkTotalBalance()
	...
	if (type === 'buy') {
		...
		// take all money items from player, check failed
		if (!removeAllMoney()) return false

		// calculate total value of change
		const change = getChange(totalBalance, cost)

		// caclutate amount of items
		const items = getMoneyItemAmount(change)
	...
	if (type === 'sell') {
		...
		// caclutate amount of items
		const items = getMoneyItemAmount(profit)
...
// calculate required amount of balance
function getCost(currentPrice: number, amount: number): number {
	// set required cost
	const cost = amount * currentPrice

	// add traiding fee
	const fee = fixDigits(cost * TRADING_FEE_RATE, 1000)
...
function getChange(totalBalance: number, cost: number): number {
	// calc change
	const change = totalBalance - cost
...
function getProfit(currentPrice: number, amount: number): number {
	...
	// subs traiding fee
	const fee = fixDigits(profit * TRADING_FEE_RATE, 1000)
...
// get amount of economy item by value
function getItemValue(value: number): number {
	const { placeholder, balance } = MONEY_ITEMS[value]

	// return calculated total balance
	return parseInt(parsePlaceholder(placeholder)) * balance
}

// calculate total balance in player inventory
function checkTotalBalance(): number {
	let totalBalance = 0

	// check all money items
	for (const item in MONEY_ITEMS) {
...
// remove all money items
function removeAllMoney(): boolean {
	const placeholder = 'checkitem_remove_lorecontains:RM-MO'

	// remove items from player's inventory
	return parsePlaceholder(placeholder) === 'yes'
}

// calculate money item to player
function getMoneyItemAmount(balance: number): { [index: string]: number } {
	// money items
	const itemAmounts: {
		[index: string]: number
	} = {
...
// give money items to player
function giveMoney(items: { [index: string]: number }): boolean {
	// money items
	const moneyItems: {
		[index: string]: string
	} = {
...
function checkBalance(args: string[]): DataType {
	...
	// get total money balance in player's inventory
	const totalBalance = checkTotalBalance()
	...
	if (typeof stockId !== 'undefined' && typeof trscAmount !== 'undefined') {
		...
		// check return type (condition: total balance >= price)
		const cond = totalBalance >= price
...
function nextUpdateETA(args: string[]): string | boolean {
	...
	const seconds = parseInt(parsePlaceholder('commandtimer_UpdateStock_nextExecution'))
