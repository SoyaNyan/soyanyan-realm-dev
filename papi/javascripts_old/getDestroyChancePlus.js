...
function getDestroyChancePlus() {
	...
	if (enchantEssence != 'false') {
		...
		var placeholder = 'checkitem_remove_inslot:' + essenceData.slot
		var essenceCheck = PlaceholderAPI.static.setPlaceholders(BukkitPlayer, '%' + placeholder + '%')
		if (essenceCheck == 'yes') {
			chanceBoost = essenceData.chance
		}
	}

	var enchantProtect = PlaceholderAPI.static.setPlaceholders(
		BukkitPlayer,
		'%javascript_apply_protect_scroll%'
	)
	var protectChance = enchantProtect == '1' ? 0 : 1
	...
	enchantData.forEach(function (item) {
		...
		if (enchantId == enchantment) {
			...
			destroyChance = upgradeFailChance * ENCHANT_UPGRADE_SETTINGS.destroyChance[enchantLevel + 1]
		}
	})

	return Math.floor(destroyChance) * protectChance + ''
}

getDestroyChancePlus()
